...
void ETH_StructInit(ETH_InitTypeDef* ETH_InitStruct)
{
  ...
  ETH_InitStruct->ETH_AutoNegotiation = ETH_AutoNegotiation_Enable; 			//ʹ������Ӧģʽ       
  /* MAC watchdog enabled: cuts-off long frame */
  ETH_InitStruct->ETH_Watchdog = ETH_Watchdog_Enable;										 	//ʹ�ܿ��Ź�
  /* MAC Jabber enabled in Half-duplex mode */					
  ETH_InitStruct->ETH_Jabber = ETH_Jabber_Enable;       									//ʹ��Jabber                                                
  /* Ethernet interframe gap set to 96 bits */
  ETH_InitStruct->ETH_InterFrameGap = ETH_InterFrameGap_96Bit;  					//����֡���Ϊ96bit                                                                                                                          
  /* Carrier Sense Enabled in Half-Duplex mode */ 
  ETH_InitStruct->ETH_CarrierSense = ETH_CarrierSense_Enable; 						//��˫��ģʽ��ʹ���ز���������                               
  /* PHY speed configured to 100Mbit/s */
  ETH_InitStruct->ETH_Speed = ETH_Speed_100M; 														//PHY���ٶ�Ϊ100M
  /* Receive own Frames in Half-Duplex mode enabled */
  ETH_InitStruct->ETH_ReceiveOwn = ETH_ReceiveOwn_Enable;                	//��˫��ģʽ���������� own frame
  /* MAC MII loopback disabled */ 
  ETH_InitStruct->ETH_LoopbackMode = ETH_LoopbackMode_Disable; 						//�ر�MII�ӿڵķ�������             
  /* Full-Duplex mode selected */
  ETH_InitStruct->ETH_Mode = ETH_Mode_FullDuplex;   											//ʹ��ȫ˫��ģʽ                   
  /* IPv4 and TCP/UDP/ICMP frame Checksum Offload disabled */
  ETH_InitStruct->ETH_ChecksumOffload = ETH_ChecksumOffload_Disable;   		//�ر�ipv4��TCP/UDP/ICMP��֡У���ж��                                                          
  /* Retry Transmission enabled for half-duplex mode */ 
  ETH_InitStruct->ETH_RetryTransmission = ETH_RetryTransmission_Enable;   //������˫��ģʽ�µ����Դ��书��                                                                              
  ...
  ETH_InitStruct->ETH_BackOffLimit = ETH_BackOffLimit_10;     						//���ð�˫��ģʽ�µ�����ش������¼�10 slot times
  /* half-duplex mode Deferral check disabled */
  ETH_InitStruct->ETH_DeferralCheck = ETH_DeferralCheck_Disable;          //�رհ�˫��ģʽ�µ���ʱ��鹦��                                                                                                      
  /* Receive all frames disabled */ 
  ETH_InitStruct->ETH_ReceiveAll = ETH_ReceiveAll_Disable;								//��ֹ��������֡
  /* Source address filtering (on the optional MAC addresses) disabled */
  ETH_InitStruct->ETH_SourceAddrFilter = ETH_SourceAddrFilter_Disable;   	//�ر�MAC��ַ��Դ��ַ���˹���
  /* Do not forward control frames that do not pass the address filtering */
  ETH_InitStruct->ETH_PassControlFrames = ETH_PassControlFrames_BlockAll; //
  /* Disable reception of Broadcast frames */
  ETH_InitStruct->ETH_BroadcastFramesReception = ETH_BroadcastFramesReception_Disable;//��ֹ�������еĹ㲥֡
  /* Normal Destination address filtering (not reverse addressing) */
  ETH_InitStruct->ETH_DestinationAddrFilter = ETH_DestinationAddrFilter_Normal;			//������Զ�˵�ַ����
  ...
  ETH_InitStruct->ETH_HashTableHigh = 0x0;   											//��ʼ��HASH���ĸ�λ�Ĵ���             
  ETH_InitStruct->ETH_HashTableLow = 0x0;                     		//��ʼ��HASH���ĵ�λλ�Ĵ���  
  ...
  ETH_InitStruct->ETH_DropTCPIPChecksumErrorFrame = ETH_DropTCPIPChecksumErrorFrame_Disable; //�رն���TCP/IP����֡
  /* Store and forward mode enabled for receive */
  ETH_InitStruct->ETH_ReceiveStoreForward = ETH_ReceiveStoreForward_Enable;       //�����������ݵĴ洢ת������
  ...
  ETH_InitStruct->ETH_TransmitStoreForward = ETH_TransmitStoreForward_Enable;   	//��������ģʽ�Ĵ洢ת������
  /* Threshold TXFIFO level set to 64 bytes (used when threshold mode is enabled) */
  ETH_InitStruct->ETH_TransmitThresholdControl = ETH_TransmitThresholdControl_64Bytes;  //������ֵģʽ�µķ���FIFO����ֵΪ64�ֽ�
  /* Disable forwarding frames with errors (short frames, CRC,...)*/
  ETH_InitStruct->ETH_ForwardErrorFrames = ETH_ForwardErrorFrames_Disable;   //��ֹת������֡
  /* Disable undersized good frames */
  ETH_InitStruct->ETH_ForwardUndersizedGoodFrames = ETH_ForwardUndersizedGoodFrames_Disable; //��ת����С�ĺ�֡
  /* Threshold RXFIFO level set to 64 bytes (used when Cut-through mode is enabled) */
  ETH_InitStruct->ETH_ReceiveThresholdControl = ETH_ReceiveThresholdControl_64Bytes;    //����ֱͨģʽ�µķ���FIFO��ֵΪ64�ֽ�                        
  /* Disable Operate on second frame (transmit a second frame to FIFO without 
  waiting status of previous frame*/                           
  ETH_InitStruct->ETH_SecondFrameOperate = ETH_SecondFrameOperate_Disable;							//�رմ����ڶ�֡����
...
void ETH_MACAddressConfig(uint32_t MacAddr, uint8_t *Addr)
{
  ...
  tmpreg = ((uint32_t)Addr[5] << 8) | (uint32_t)Addr[4]; //�������ѡ���MAC��ַ�ĸ�λ�Ĵ���ֵ
																												 //��ETH_MACA0HR��16λֵ
  /* Load the selected MAC address high register */
  (*(__IO uint32_t *) (ETH_MAC_ADDR_HBASE + MacAddr)) = tmpreg; //��������ĸ�λֵд��ETH_MAC0HR��
  /* Calculate the selected MAC address low register */
	//�������ѡ���MAC��ַ�ĵ�λ�Ĵ���ֵ,��ETH_MACA0LR��16λֵ
  tmpreg = ((uint32_t)Addr[3] << 24) | ((uint32_t)Addr[2] << 16) | ((uint32_t)Addr[1] << 8) | Addr[0];
 
  /* Load the selected MAC address low register */
  (*(__IO uint32_t *) (ETH_MAC_ADDR_LBASE + MacAddr)) = tmpreg;//��������ĵ�λֵд��ETH_MAC0LR��
}
